...
uniform vec4 U_Setting;//x(smooth_level),y(width),z(height)
uniform vec4 U_MoPiArea;//x(pixel),y(pixel),radius(texcoord)
...
bool IsInCircle(vec3 circle,vec2 texcoord){
    float distance_to_circle_center=length(texcoord-circle.xy);
    if(distance_to_circle_center<circle.z){
        return true;
    }
    return false;
}
void main(){
    vec3 circle=vec3(U_MoPiArea.x/U_Setting.y,U_MoPiArea.y/U_Setting.z,U_MoPiArea.z);//0.0~1.0
    vec3 center_color=texture2D(U_Texture,V_Texcoord.xy).rgb;
    if(IsInCircle(circle,V_Texcoord.xy)){
        ...
        gl_FragColor=vec4(center_color, 1.0);
    }
}