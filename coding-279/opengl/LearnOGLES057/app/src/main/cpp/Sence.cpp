...
Camera camera;
Shader *ui_shader,*mopi_shader;
Material*ui_material,*mopi_material;
Geometry *quad;
...
void InitFBONodes(){
    mopi_shader=new Shader;
    mopi_shader->InitStandardShader("ui.vs","MoPi3.fs");

    mopi_material=new Material;
    mopi_material->mShader=mopi_shader;
    mopi_material->SetTexture("U_Texture",CreateTextureFromFile("jason.jpg"));
    float setting[]={2.0f,256.0f,256.0f,0.0};
    float area[]={128.0f,128.0f,0.1f,0.0f};
    mopi_material->SetVec4("U_Setting",setting);
    mopi_material->SetVec4("U_MoPiArea",area);

    SceneNode*node=new SceneNode;
    node->mMaterial=mopi_material;
    node->mGeometry=quad;
...
extern "C" JNIEXPORT void JNICALL Java_cn_battlefire_learnogles_MainActivity_OnViewportChanged(
    ...
    camera.mProjectionMatrix=glm::ortho(-128.0f,128.0f,-128.0f,128.0f,0.1f,100.0f);
    fbo[0].Init(256,256);
    fbo[1].Init(256,256);
    for (int i = 0; i < 3; ++i) {
        if(i>0){
            mopi_material->SetTexture("U_Texture",fbo[1].mColorBuffer);
        }
        fbo[0].BeginDraw();
        if(sRootNode!=nullptr){
        ...
        fbo[0].EndDraw();
        mopi_material->SetTexture("U_Texture",fbo[0].mColorBuffer);
        fbo[1].BeginDraw();
        if(sRootNode!=nullptr){
            sRootNode->Update(GetFrameTime());
            sRootNode->Render(&camera);
    ...
    ui_material->SetTexture("U_Texture",fbo[1].mColorBuffer);
}
