...
GLuint CreateProcedureTexture(int size){
    unsigned char * rgba_pixel=new unsigned char[size*size*4];
    float half_size=float(size)/2.0f;
    float max_distance_to_center=sqrtf(half_size*half_size*2.0f);
    float center_x=half_size;
    float center_y=half_size;
    for (int y = 0; y < size; ++y) {
        for (int x = 0; x < size; ++x) {
            int current_pixel_index=y*size+x;
            int current_pixel_data_offset=current_pixel_index*4;
            rgba_pixel[current_pixel_data_offset]=255;
            rgba_pixel[current_pixel_data_offset+1]=255;
            rgba_pixel[current_pixel_data_offset+2]=255;
            float delta_x=float(x)-center_x;
            float delta_y=float(y)-center_y;
            float distance_to_center=sqrtf(delta_x*delta_x+delta_y*delta_y);
            float alpha=(1.0f-distance_to_center/max_distance_to_center);
            alpha=powf(alpha,8.0f);
            alpha=alpha>1.0f?1.0f:alpha;
            rgba_pixel[current_pixel_data_offset+3]=(unsigned char)(alpha*255.0f);
        }
    }
    GLuint texture=CreateTexture2D(rgba_pixel,size,size,GL_RGBA,GL_RGBA);
    delete [] rgba_pixel;
    return texture;
}