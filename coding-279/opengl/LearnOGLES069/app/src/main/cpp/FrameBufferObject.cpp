...
void FrameBufferObject::Init(int width, int height) {
    ...
    glFramebufferTexture2D(GL_FRAMEBUFFER,GL_COLOR_ATTACHMENT0,GL_TEXTURE_2D,mColorBuffer0,0);
    glFramebufferTexture2D(GL_FRAMEBUFFER,GL_COLOR_ATTACHMENT1,GL_TEXTURE_2D,mColorBuffer1,0);
    glFramebufferTexture2D(GL_FRAMEBUFFER,GL_COLOR_ATTACHMENT2,GL_TEXTURE_2D,mColorBuffer2,0);
    glFramebufferTexture2D(GL_FRAMEBUFFER,GL_COLOR_ATTACHMENT3,GL_TEXTURE_2D,mColorBuffer3,0);
    ...
    GLenum draw_targets[]={GL_COLOR_ATTACHMENT0,GL_COLOR_ATTACHMENT1,GL_COLOR_ATTACHMENT2,GL_COLOR_ATTACHMENT3};
    glDrawBuffers(4,draw_targets);
    glBindFramebuffer(GL_FRAMEBUFFER,mFrameBufferObjectBeforeSwitch);
}
void FrameBufferObject::BeginDraw() {
    glGetIntegerv(GL_FRAMEBUFFER_BINDING,&mFrameBufferObjectBeforeSwitch);
    glBindFramebuffer(GL_FRAMEBUFFER,mFrameBufferObject);
    glViewport(0,0,mResolutionWidth,mResolutionHeight);
